...
subroutine Conservative2Primitive(CCTK_ARGUMENTS)
  ...
  CCTK_REAL, DIMENSION(cctk_lsh(1),cctk_lsh(2),cctk_lsh(3)) :: dens_interp, tau_interp, scon1_interp, scon2_interp, scon3_interp
      ...
      do i = 1, nx
        ...
        if(evolve_Y_e.ne.0) then
         ...
         if(evolve_temper.eq.0) then
          ...
          perc_dens  = diff_dens  / dens_de_avg(i,j,k) * 100
          perc_tau   = diff_tau   / tau_de_avg(i,j,k) * 100
          perc_scon1 = diff_scon1 / scon1_de_avg(i,j,k) * 100
          perc_scon2 = diff_scon2 / scon2_de_avg(i,j,k) * 100
          perc_scon3 = diff_scon3 / scon3_de_avg(i,j,k) * 100
          ...
          if (ieee_is_nan(dens_de_avg(i,j,k)) .or. &
            ...
            if(wham_deavg_diff .lt. 5.0d0) then
              !de-avg
              call Con2Prim_pt(int(cctk_iteration,ik),int(i,ik),int(j,ik),int(k,ik),&
                 ...
                 z(i,j,k),r(i,j,k),epsnegative,GRHydro_rho_min,pmin, epsmin, & 
                 GRHydro_reflevel, GRHydro_C2P_failed(i,j,k))
            else if(wham_deavg_diff .ge. 10.0d0) then
              !avg
              call Con2Prim_pt_avged(int(cctk_iteration,ik),int(i,ik),int(j,ik),int(k,ik),&
                 GRHydro_eos_handle, dens(i,j,k),scon(i,j,k,1),scon(i,j,k,2), &
                 scon(i,j,k,3),tau(i,j,k),rho(i,j,k),vup(i,j,k,1),vup(i,j,k,2), &
                 ...
                 z(i,j,k),r(i,j,k),epsnegative,GRHydro_rho_min,pmin, epsmin, & 
                 GRHydro_reflevel, GRHydro_C2P_failed(i,j,k))
            else if((wham_deavg_diff .lt. 10.0d0) .and. (wham_deavg_diff .gt. 5.0d0)) then
              !interp
              interpolation_factor = (10.0d0 - wham_deavg_diff) / 5.0d0
              GRHydro_C2P_failed(i,j,k) = 0
              !Calculate interp'd values to use in c2p
              dens_interp(i,j,k)  = interpolation_factor * dens_de_avg(i,j,k)   + (1.0d0 - interpolation_factor) * dens(i,j,k)
              tau_interp(i,j,k)   = interpolation_factor * tau_de_avg(i,j,k)    + (1.0d0 - interpolation_factor) * tau(i,j,k)
              scon1_interp(i,j,k) = interpolation_factor * scon1_de_avg(i,j,k)  + (1.0d0 - interpolation_factor) * scon(i,j,k,1)
              scon2_interp(i,j,k) = interpolation_factor * scon2_de_avg(i,j,k)  + (1.0d0 - interpolation_factor) * scon(i,j,k,2)
              scon3_interp(i,j,k) = interpolation_factor * scon3_de_avg(i,j,k)  + (1.0d0 - interpolation_factor) * scon(i,j,k,3)

              call Con2Prim_pt(int(cctk_iteration,ik),int(i,ik),int(j,ik),int(k,ik),&
                 GRHydro_eos_handle, dens_interp(i,j,k),scon1_interp(i,j,k),scon2_interp(i,j,k), &
                 scon3_interp(i,j,k),tau_interp(i,j,k),rho(i,j,k),vup(i,j,k,1),vup(i,j,k,2), &
                 vup(i,j,k,3),eps(i,j,k),press(i,j,k),w_lorentz(i,j,k), &
                 uxx,uxy,uxz,uyy,uyz,uzz,sdetg(i,j,k),x(i,j,k),y(i,j,k), &
                 z(i,j,k),r(i,j,k),epsnegative,GRHydro_rho_min,pmin, epsmin, & 
                 GRHydro_reflevel, GRHydro_C2P_failed(i,j,k))
            ...
            if(GRHydro_C2P_failed(i,j,k).ne.0) then
              ...
              write(warnline,'(a28)') 'de avg failed, switching'
              GRHydro_C2P_failed(i,j,k) = 0
              call Con2Prim_pt_avged(int(cctk_iteration,ik),int(i,ik),int(j,ik),int(k,ik),&
                 GRHydro_eos_handle, dens(i,j,k),scon(i,j,k,1),scon(i,j,k,2), &
                 scon(i,j,k,3),tau(i,j,k),rho(i,j,k),vup(i,j,k,1),vup(i,j,k,2), &
          ...
          if (ieee_is_nan(rho(i,j,k)) .or. &
              ...
              ieee_is_nan(press(i,j,k))) then
                ...
                write(warnline,'(a28)') 'Somewhere we had a NaN, reverting back to old values and switching back to base Con2Prim'

            !Reset everything back to input values, in case we messed up somewhere
                dens(i,j,k) = dens_in
                scon(i,j,k,1) = sx_in
                scon(i,j,k,2) = sy_in
                scon(i,j,k,3) = sz_in
                ...
                vup(i,j,k,3) = velz_in
                eps(i,j,k) = eps_in
                press(i,j,k) = press_in
                w_lorentz(i,j,k) = w_lorentz_in

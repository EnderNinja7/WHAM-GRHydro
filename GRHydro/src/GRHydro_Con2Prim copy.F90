    ...
    do j = 1, ny 
      do i = 1, nx
        ...
        if (GRHydro_enable_internal_excision /= 0 .and. (hydro_excision_mask(i,j,k) .gt. 0)) then
           ...
           SET_ATMO_MIN(dens_de_avg(i,j,k), sdetg(i,j,k)*GRHydro_rho_min, r(i,j,k)) !sqrt(det)*GRHydro_rho_min !/(1.d0+GRHydro_atmo_tolerance)
           SET_ATMO_MIN(rho(i,j,k), GRHydro_rho_min, r(i,j,k)) !GRHydro_rho_min
           scon(i,j,k,:) = 0.d0
           scon1_de_avg(i,j,k) = 0.0d0
           scon2_de_avg(i,j,k) = 0.0d0
           scon3_de_avg(i,j,k) = 0.0d0
           vup(i,j,k,:) = 0.d0
           ...
           tau_de_avg(i,j,k)  = sdetg(i,j,k) * (rho(i,j,k)+rho(i,j,k)*eps(i,j,k)) - dens_de_avg(i,j,k)
...
!!$        end if
        ...
        if(evolve_Y_e.ne.0) then
         ...
         IF_BELOW_ATMO(dens(i,j,k), sdetg(i,j,k)*GRHydro_rho_min, GRHydro_atmo_tolerance, r(i,j,k)) then
          ...
          !Same fix for de-averaged quantities
          IF_BELOW_ATMO(dens_de_avg(i,j,k), sdetg(i,j,k)*GRHydro_rho_min, GRHydro_atmo_tolerance, r(i,j,k)) then
           SET_ATMO_MIN(dens_de_avg(i,j,k), sdetg(i,j,k)*GRHydro_rho_min, r(i,j,k)) !sqrt(det)*GRHydro_rho_min !/(1.d0+GRHydro_atmo_tolerance)
           SET_ATMO_MIN(rho(i,j,k), GRHydro_rho_min, r(i,j,k)) !GRHydro_rho_min
           scon1_de_avg(i,j,k) = 0.0d0
           scon2_de_avg(i,j,k) = 0.0d0
         ...
         if(evolve_temper.eq.0) then
          ...
          if (wham_deavg_diff .lt. 0.05d0) then
            ...
            call Con2Prim_pt(int(cctk_iteration,ik),int(i,ik),int(j,ik),int(k,ik),&
                 GRHydro_eos_handle, dens_de_avg(i,j,k),scon1_de_avg(i,j,k,1),scon2_de_avg(i,j,k,2), &
                 scon3_de_avg(i,j,k,3),tau_de_avg(i,j,k),rho(i,j,k),vup(i,j,k,1),vup(i,j,k,2), &
